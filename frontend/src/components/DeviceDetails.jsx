...
const DeviceDetails = ({ device, timeline }) => (
  <div className="space-y-6">
    <div className="rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-[0_20px_45px_rgba(15,23,42,0.08)]">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h3 className="text-2xl font-semibold text-slate-900">{device.name}</h3>
    ...
    <div className="rounded-3xl border border-slate-200 bg-white/95 p-6">
      ...
      <LineChartComponent labels={timeline.labels} dataPoints={timeline.values} />
    </div>
    ...
    <div className="rounded-3xl border border-slate-200 bg-white/95 p-6">
      ...
      <div className="space-y-4">
        {device.anomalies.map((event) => (
          <div key={event.time} className="flex items-center justify-between rounded-2xl border border-slate-100 p-4">
            <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-semibold uppercase tracking-wide text-rose-500">Critical</span>
          </div>
        ))}
      </div>
    </div>
